...
export default function SignupPage() {
  ...
  const pageVariants = {
    hidden: { opacity: 0, scale: 0.95, filter: 'blur(10px)', skewY: -2 },
    visible: { 
      opacity: 1, scale: 1, filter: 'blur(0px)', skewY: 0,
      transition: { duration: 0.8, ease: [0.16, 1, 0.3, 1] as const, staggerChildren: 0.1 } 
    },
    exit: { opacity: 0, scale: 1.05, filter: 'blur(10px)', skewY: 2, transition: { duration: 0.6 } }
  }
      ...
      >
        ...
        >
          <div style={{
            ...
            transform: 'scaleX(-1)' // Mirror image for visual variety
          }} />
          <div style={{
            position: 'absolute', inset: 0,
            background: 'linear-gradient(to left, #030303 0%, transparent 15%, transparent 85%, rgba(0,0,0,0.4) 100%)'
          }} />
        </motion.div>

        {/* ── Right Side: Form (Liquid Glass) ── */}
        <div style={{
          flex: '1 1 50%',
          display: 'flex',
          ...
          <motion.div variants={formVariants} style={{ width: '100%', maxWidth: '440px' }}>
            ...
            <div style={{
              ...
              <motion.form variants={itemVariants} onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                
                <div style={{ display: 'flex', gap: '12px' }}>
                  <FieldWrap icon={<User size={16} />} style={{ flex: 1 }}>
                    <input
                      type="text"
              ...
              <motion.p variants={itemVariants} style={{ textAlign: 'center', marginTop: '36px', fontSize: '14px', color: 'rgba(255,255,255,0.3)' }}>
                ...
                <Link href="/login" style={{ color: '#fff', fontWeight: 500, textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.3)', paddingBottom: '2px', transition: 'all 0.2s' }}>
                  Sign in
                </Link>
              </motion.p>
...
function FieldWrap({ icon, children, style = {} }: { icon: React.ReactNode; children: React.ReactNode; style?: React.CSSProperties }) {
  return (
  ...
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
      <div style={{ display: 'flex', gap: '4px', marginTop: '4px' }}>
        {[1, 2, 3, 4].map(i => (
          <div key={i} style={{ flex: 1, height: '3px', borderRadius: '2px', background: i <= score ? color : 'rgba(255,255,255,0.07)', transition: 'background 0.3s' }} />
        ))}
      </div>
      <p style={{ fontSize: '12px', marginTop: '6px', color, fontWeight: 500, transition: 'color 0.3s' }}>{label}</p>
    </motion.div>
  )
}

function GoogleSignInWrapper({ onSuccess, label = 'Continue with Google' }: { onSuccess: (res: any) => void; label?: string }) {
  return (
